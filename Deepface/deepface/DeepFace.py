...
def build_model(model_name):
    ...
    models = {
        ...
        "Race": Race.loadModel,
        "resnet34_finetune": Resnet34_finetune.loadModel,
        "resnet34_normhead": Resnet34_normhead.loadModel,
    }
...
def find(
    ...
    for target_img, target_region, _ in target_objs:
        ...
        distances = []
        if distance_metric == "proba":
            source = np.array(df[f"{model_name}_representation"].to_numpy())
            source = np.array(source.tolist())
            temps = np.sqrt(np.sum(np.multiply(source, source), axis=1)).reshape(-1,1)
            temps = np.repeat(temps, 512, 1)
            source = source / temps
            target = np.array(target_representation)
            target = dst.l2_normalize(target)
            print(np.sum(source**2, axis=1))
            print(np.sum(target**2))
            distances = dst.findProba(np.abs(source-target))
        else:
        ...
        if distance_metric == "proba":
            result_df = result_df.sort_values(
                by=[f"{model_name}_{distance_metric}"], ascending=False
